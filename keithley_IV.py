...
class IVCurve:
    def __init__ (self, master):
        ...
        tk.Label(ctr_left,text ="Analyse IV Curve",bg=bkgcolor,fg = fontcolor).grid(row=18,sticky="w")
        tk.Button(ctr_left, text='Analyse IV', command=self.IVLinearFittingAnalysis,highlightbackground=bkgcolor).grid(row=19,column=0,sticky="ew")
    ...
    def IVLinFitCleanData(self,inputVoltages, inputCurrents):
        #Added by Pritindra
    
        voltages = np.array(inputVoltages)
        currents = np.array(inputCurrents)
    
        ind_lowcut = np.argwhere(voltages>=30)[0][0]
        try: ind_uppcut = np.argwhere(voltages>=60)[0][0]
        except: ind_uppcut = np.argwhere(voltages>=np.max(inputVoltages)-1)[0][0]
        
        voltagesCut = np.log(np.array(inputVoltages[ind_lowcut:ind_uppcut]))
        currentsCut = np.log(np.array(inputCurrents[ind_lowcut:ind_uppcut]))

        logSlope, logIntercept, logRcoeff, p, logSE = sp.stats.linregress(voltagesCut, currentsCut)

        currentResidue = currentsCut-voltagesCut*logSlope-logIntercept
    
        variance = np.std(currentResidue)
    
        index = []
    
        for i in range(len(currentResidue)):
            if currentResidue[i]>3*variance or currentResidue[i]<-3*variance: index.append(i)
    
        if len(index) == 0: return voltages,currents
    
        index = ind_lowcut + np.array(index)
        voltages = np.delete(voltages, index)
        currents = np.delete(currents, index)
    
        return voltages,currents


    def IVLinearFittingAnalysis(self):
        #Added by Pritindra
        ...
        voltages,currents = self.IVLinFitCleanData(voltages,currents)
        voltages,currents = self.IVLinFitCleanData(voltages,currents)
        voltages,currents = self.IVLinFitCleanData(voltages,currents)
        
        try: ind_lowcut = np.argwhere(voltages>=30)[0][0]
        except: ind_lowcut = np.argwhere(voltages>=30)[0][0]
        try: ind_uppcut = np.argwhere(voltages>=65)[0][0]
        except: ind_uppcut = np.argwhere(voltages>=IVdata[-1,0]-1)[0][0]
        
        voltages = np.log(voltages[ind_lowcut:ind_uppcut])
        currents = np.log(currents[ind_lowcut:ind_uppcut])
        standardError = np.log10(np.linalg.det(np.cov([voltages, currents]))/np.var(voltages))/2
        
        if standardError > -2.1: 
            self.v2.set("Standard error of linear fit  = {}. log10(Standard error of linear fit)  = {}. Noisy IV or Early Onset breakdown".format(10**(standardError),standardError))
            
        if standardError < -2.1: 
            self.v2.set("Standard error of linear fit  = {}. log10(Standard error of linear fit)  = {}. IV linear fit passed".format(10**(standardError),standardError))
    ...
    def TakeIVCurve(self):
        ...
        while(temp < EV)&(self.STOP==False):
            ...
            self.keithley.source_voltage = self.keithley.source_voltage + float(self.step_size.get())
...
##            temp=self.keithley.voltage
    ...
    def connect(self):
        try:
            adapter = 'USB0::0x05E6::0x2450::04610524::INSTR'#VISAAdapter('<usb address here>') # should look something like this 'USB0::0x05E6::0x2450::04490665::INSTR'
            ...
            self.keithley.compliance_current = 100e-6
